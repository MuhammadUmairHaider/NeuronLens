...
class SoftMaskLayer(nn.Module):
    def __init__(self, lower_bound, upper_bound, factor=0.5):
        super().__init__()
    ...
    def forward(self, x):
        lb = self.lower_bound.to(device=x.device, dtype=x.dtype).view(1, 1, -1)
        ub = self.upper_bound.to(device=x.device, dtype=x.dtype).view(1, 1, -1)
        center = (lb + ub) / 2
        half_range = (ub - lb) / 2

        # Compute normalized distance from center (0 at center, 1 at boundary)
        dist = (x - center).abs() / (half_range + 1e-6)
        dist = torch.clamp(dist, 0, 1)

        # Compute scaling: 0 at center, factor at boundary
        scaling = dist * self.factor

        # Apply scaling only inside the range, leave others untouched
        mask = (x >= lb) & (x <= ub)
        return torch.where(mask, x * scaling, x)

    def set_perms(self, lower_bound, upper_bound, factor):
        self.lower_bound = lower_bound
        self.upper_bound = upper_bound
        self.factor = factor
    ...
    def forward(
        ...
        **kwargs,
        ...
        if(mask_layer is not None):
            hidden_states = mask_layer(hidden_states)
        hidden_states = residual + hidden_states
...
# class MaskLayer(nn.Module):
#     def __init__(self, lower_bound, upper_bound, factor=0.12):
#         super().__init__()
#         self.lower_bound = lower_bound
#         self.upper_bound = upper_bound
#         self.factor = 0.12
...
#         mask = (x >= lb) & (x <= ub)
#         return torch.where(mask, x * self.factor, x)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        if config.mask_layer_type == 'hard':
            self.mask_layer = MaskLayer(torch.tensor(float('inf')), torch.tensor(float('-inf')), torch.tensor(0.0))
        elif config.mask_layer_type == 'soft':
            self.mask_layer = SoftMaskLayer(torch.tensor(float('inf')), torch.tensor(float('-inf')), torch.tensor(0.0))#softmask run
